...
| 指标 | 数值 | 行业基准 | 解读 |
|---|---|---|---|
| PEG | X.XX | {行业} P50=1.X | 高于/低于行业中位数 |
| 52周位置 | `[████░░░]` XX% | — | 接近高点/低点，追高/抄底需谨慎 |
| 分析师 (美股) | Buy/Hold/Sell | — | 目标$XXX · 🟢/🔴 ±XX% |

**📋 第二档：质地指标**

| 指标 | 数值 | 解读 |
|---|---|---|
| PE / EPS | XX.X / $X.XX | 估值水平 |
| ROA / ROE | XX% / XX% | 用钱效率 |
| 利润率 | 毛利X% / 净利X% | 赚钱效率 |
| FCF / 负债 | $XX / XX% | 财务健康 |
| 市值 | X.XX万亿 | 体量 |
...
所有基本面指标分为两档。第一档直接关系到"现在该不该买"，**必须包含行业相对位置**；第二档只判断"公司好不好"，不跟行业比。

---

#### 第一档：核心决策指标（必须展示行业相对位置）

格式要求：**[数值] + [行业P25-P50-P75区间] + [白话解读]**

##### PEG（市盈增长比）

```
格式：PEG X.XX | {行业名} P25=0.X P50=1.X P75=2.X | 解读
示例：PEG 2.41，高于 Consumer Electronics 行业中位数 1.8，增长已被股价提前消化
```

- PEG = PE ÷ 盈利增速。PEG=1 算合理，<1 偏便宜，>1 偏贵
- **PEG 必须对标行业**——不同行业的 PEG 基准完全不同（银行 PEG 0.8 就算贵，芯片 PEG 1.5 可能还便宜）

##### 52周位置

```
格式：52周位置 XX% | 52周高 $XXX / 低 $XXX | [████░░░] | 解读
示例：52周位置 83%，接近52周高点 $317，追高需谨慎
```
...
```
格式：评级 (Buy/Hold/Sell) | 目标均价 $XXX | 潜在空间 ±XX% | 解读
示例：Buy · 目标$314 · 🟢 +5.9% · 温和看涨但空间有限
```
...
#### 第二档：质地指标（仅数值 + 白话解读，不做行业对比）

这些指标回答"公司好不好"，但不回答"现在该不该买"：

| 指标 | 格式示例 | 为什么是第二档 |
|------|---------|---------------|
| **ROA** | ROA 26.2% — 每$100资产赚$26，资产效率极强 | 好公司≠好买点 |
| **利润率** | 毛利47.9% / 净利27.2% — 赚钱机器 | 质地判断，非时机判断 |
| **自由现金流** | $1,011亿 — 确认公司真有钱，不是纸面富贵 | 财务体检，非买卖信号 |
| **EPS** | $8.26 — 每股一年赚$8.26 | 基础数据 |
| **负债权益比 / 流动比率** | 79.5 / 1.07 — 财务安全 | 风控参考 |

> 💡 第二档指标即使数值再漂亮（ROA 50%+、毛利 70%+），也**不能单独作为买卖理由**。股价可能已经充分反映了这些优质特征。

---

#### 行业基准数据查找规则

1. 从脚本输出中提取 `sector` 和 `industry` 字段（Yahoo `info` 提供）
2. 查 `industry_benchmarks.json` 中匹配的行业 (key: `{sector} - {industry}`)
3. 若匹配命中 → 输出 `P25/P50/P75` 并做相对解读
4. 若没有匹配 → PEG 仅输出原始数值 + "行业对比数据暂缺"
5. **禁止**现场拉取同行业所有个股做实时 PEG 计算

**行业基准文件**：`industry_benchmarks.json`（周更新，覆盖 12 个常见行业）
